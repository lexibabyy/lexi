//+------------------------------------------------------------------+
//|                                           LexiAutoTraderBTC.mq5   |
//|   Automated MetaTrader 5 Expert Advisor                          |
//|                                                                  |
//|   Designed for BTCUSD (Bitcoin) — attach it to a BTCUSD chart.   |
//|   Uses ATR-based stop and ATR-based grid spacing so it adapts    |
//|   to Bitcoin's large price and high volatility automatically.    |
//|                                                                  |
...
input double   InpSLAtrMult          = 2.0;       // Stop-loss = ATR x this (wider for BTC volatility)
input double   InpFixedSLPoints      = 0;         // Fixed SL in points (0 = use ATR; BTC should use ATR)
input double   InpTPAtrMult          = 4.0;       // Take-profit = ATR x this (wide = let it run)
input double   InpMinLot             = 0.01;      // Minimum lot (small — BTC notional is huge per lot)
input double   InpMaxLot             = 0.01;      // Maximum lot (small — BTC notional is huge per lot)
...
input double   InpGridStepPoints     = 100;       // Min price gap (points) — used only if ATR step = 0
input double   InpGridStepATR        = 0.5;       // Grid gap as ATR multiple (adapts to BTC; 0=use points)
...
//--- Inputs: General ------------------------------------------------
input int      InpMaxOpenPositions   = 3;         // Max simultaneous positions (smaller — BTC is wild)
input long     InpMagicNumber        = 532024;    // Unique ID (different from gold bot, so they don't clash)
...
void TryGridEntry()
  {
   ...
   double step  = (InpGridStepATR > 0.0)
                  ? CurrentATR() * InpGridStepATR
                  : InpGridStepPoints * SymbolInfoDouble(_Symbol, SYMBOL_POINT);
